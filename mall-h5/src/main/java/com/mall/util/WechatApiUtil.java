...
public class WechatApiUtil {
	 ...
	 * 炎宝公众号
	 * */
	private static final String YANBAO_APPID = "wx32266be91b9e0a8f";

	private static final String YANBAO_SECRET = "d9b7e01273e3cf705e4c4bc66bd27514";
	...
	public static String authorizeH5(String redirect_uri, String scope, String state) {
		...
		return AUTHORIZE_URL.replace("APPID", YANBAO_APPID).replace("REDIRECT_URI", redirect_uri).replace("SCOPE", scope).replace("STATE", state);
	}
	...
	public static JSONObject accessTokenH5(String code, String state) {
		...
		JSONObject result = null;
		String requestUrl = ACCESS_TOKEN_URL.replace("APPID", YANBAO_APPID).replace("SECRET", YANBAO_SECRET).replace("CODE", code);
		JSONObject json = HttpsUtil.doGetRequest(requestUrl);
		if (!json.isEmpty() && json.get("errcode") == null) {
			result = userinfo(getJsonByKey(json, "openid"), getJsonByKey(json, "access_token"));
		}
